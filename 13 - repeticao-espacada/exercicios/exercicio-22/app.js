...
const growingOrdered = characters.sort((a,b) => a.id - b.id) 
console.log(growingOrdered)
...
const growingNumbers = numbers.sort((a,b) => a - b)
console.log(growingNumbers)
...
const zToAOrderedPeople = people.sort((a, b) => b.localeCompare(a))
console.log(zToAOrderedPeople)
...
const cooked = ingredients.reduce((acc, ingredient, index, obj) =>{
  acc += (ingredient[ingredient.length - 1]) === 'a' ?`${ingredient} cozida, ` : `${ingredient} cozido, ` 
  
  const lastIngredient = ingredients.length - 1 
  lastIngredient === index ? acc[acc.length - 1] = '.' : false
...
const totalPeopleAmountDisney = topBrazilmovies.filter(({distributedBy}) => distributedBy === "Disney")
.reduce((acc, {peopleAmount}) =>{ acc += peopleAmount
return acc
}, 0)
console.log(totalPeopleAmountDisney)
...
*/
const containerListOfFilms = document.querySelector('[data-js="list-group"]')
const listOfFilmsHtmlTemplate = topBrazilmovies.reduce((acc, {title}) => acc += `<li>${title}</li>` , '')
containerListOfFilms.innerHTML = listOfFilmsHtmlTemplate
