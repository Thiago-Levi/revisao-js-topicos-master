...
const renderFunc = (acc, infoMovie) => {
  return (acc += `
  - ${infoMovie.title}, dirigido por ${infoMovie.directedBy}`);
};
const renderBestMovies = best2019Movies.reduce(
  renderFunc,
  `Segundo o site Omelete, os melhores filmes de 2019 são:`
);

console.log(renderBestMovies);
...
const youtubeUser = {
  ...
  showRecentVideos: function () {
    const renderRecentVideos = this.videos.recentVideos.reduce(
      (acc, video) =>
        (acc += `
        Title: ${video.title}`),
      `Vídeos recentes de Roger Melo:`
    );
    return renderRecentVideos;
  },
};
