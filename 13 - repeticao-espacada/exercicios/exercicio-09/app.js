...
console.log(convertToString("Revisão"));
...
const qtdCharacter = (str) => str.length;
console.log(qtdCharacter("abcde"));
...
*/
const indexOfCharacter = function (character, str) {
  return str.indexOf(character);
};
console.log(indexOfCharacter("c", "abacate"));
...
const containsInArray = function (item, array) {
  return array.includes(item);
};

console.log(containsInArray("abacaxi", ["abacaxi", 1, true, "", 100]));
...
console.log(concatArrays(["A", "B", "C"], ["D", "E", "F"]));
...
function isNull(value) {
  return value === null ? "Valor é nulo" : "Valor não é nulo";
}
...
console.log(evoqueFunc(showMyName));
...
function evoqueCallback(callback, value) {
  return tripleOf(value);
}

function tripleOf(value) {
  return value * 3;
}

console.log(evoqueCallback(tripleOf, 11));
...
numbers.forEach((element, index, obj) =>
  console.log(`O ${index + 1}º item do array ${obj} é ${element}.`)
);
...
let paragraphs = "";
review.forEach((paragraph) => (section.innerHTML += `<p>${paragraph}</p>`));
...
function likeCounter(array) {
  numbersOfLikes = array.length;
  if (numbersOfLikes === 0) {
...
console.log(
  likeCounter(["Pedro", "Thiago", "Luiz", "Laura", "Leila", "Felipe"])
);
