...
const oddVerify = (number) => {
  const isOdd = !(number % 2 === 0);
  return isOdd;
};

const oddOfRandomNumbers = randomNumbers.filter((number) => oddVerify(number));
console.log(oddOfRandomNumbers);
...
const beLow501 = (number) => number < 501;
const qtdBelow501 = crazyNumbers.reduce((acc, number) => {
  if (beLow501(number)) {
    acc++;
  }
...
const numbers = [5, 7, 3];
const squadtFunction = (number) => Math.pow(number, 2);
...
const nameOfGames = cart.reduce((acc, { name }) => {
  acc += ` - ${name}
  `;
  return acc;
}, `  `);
...
const before2000 = (release) => {
  return release < 2000;
};
const tarantinoMoviesBefore2000 = tarantinoMovies.filter(({ release }) =>
  before2000(release)
);
